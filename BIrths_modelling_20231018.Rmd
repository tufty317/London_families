...
births_and_pop_line_ind <- total_London_births_pop %>%
  mutate(Year = as.numeric(Year)) %>%
  mutate(indexed_births = (Total_births/first(Total_births))*100) %>% 
  mutate(indexed_pop = (Total_pop/first(Total_pop))*100) %>% 
  ggplot() +
  geom_line(aes(x = Year, y = indexed_births, group = 1, colour = "Indexed Births"
                # text = paste("Zone: ", Inner_factor,
                #              "<br>Year: ", Year.x,
                #              "<br>Actual births: ", round(SUM_actual_births, digits = 0))
  ), size = 1) + 
    geom_line(aes(x = Year, y = indexed_pop, group = 1, colour = "Indexed Population of potential mothers"
       # text = paste("Zone: ", Inner_factor,
       #              "<br>Year: ", Year.x,
       #              "<br>Modelled births with 2013 ASFR: ", round(SUM_ASFRconst_births, digits = 0))
  ), size = 1) + 
  ...
  theme(axis.text.x=element_text(color = "black", 
    ...
    scale_color_manual(name='', values=c('Indexed Births'= '#943fab', 
                                               'Indexed Population of potential mothers'='#5ea15d')) +
...
ggsave (filename = (paste0(chart_dir, "births_and_pop_line_ind.png")),
