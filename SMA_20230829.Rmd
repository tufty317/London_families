...
# Calculate SMA using new method at end of August 2023
...
# For 2021 in England (excluding London) and Wales, SMA is 30.6 and TFR is 1.56 (or 1.58??)

# Testing whether SMA calculated using new method gets same result (Feb '24) YES it does
EandW_SMA_allYears_Feb <- merged_EandW_births_pop_allYears %>%
  mutate(Age_mother = as.numeric(age_mother)) %>%
...
# Adding 0.5 to all ages before calculation has same effect as adding 0.5 to final result.
...
SMA_London_map2_rep <- SMA_London_geog %>%
  ggplot()+
...
ggsave (filename = "C:/Families/Charts_for_Daryl/21_I2_SMA_London_map2.svg",
         plot = SMA_London_map2_rep,
         device = "svg",
         dpi=600,
         width = 9,
...
SMA_London_EandW_line <- merged_SMA_London_EandW %>%
  ...
  mutate(year_factor = as.factor(Year)) %>%
  ggplot() +
  geom_line(aes(x = year_factor, y = EandW_SMA, group = 1, colour = "Rest of E&W", 
                text = paste("Area: Rest of England and Wales",
                             "<br>Year: ", year_factor,
                             "<br>SMA: ", round(EandW_SMA, digits = 2))   ), size = 1) + 
  geom_line(aes(x = year_factor, y = SMA, group = 1, colour = "London", 
                text = paste("Area: London", 
                             "<br>Year: ", year_factor,
                             "<br>SMA: ", round(SMA, digits = 2))   ), size = 1) + 
  theme_gla() +
...
ggsave (filename = (paste0(chart_dir, "SMA_London_zone_EandW_lines.png")),
...
```{r fig_London_EandW_SMA_B, fig.height = 5.56, fig.width = 9,  echo=FALSE, warning=FALSE, message=FALSE}


SMA_London_zone_EandW_lines_rep <-
   ggplot() +
  theme_gla() +
  geom_line(data = SMA_Inner_London, aes(x = year_numeric, y = SMA,  group = 1, color = '#d82222' ), size = 2) +
   geom_label_repel(data = SMA_Inner_London, aes(x = year_numeric, y = SMA, label = label), nudge_x = 0, nudge_y = 0.4, color = '#d82222') +
...
ggsave (filename = "C:/Families/Charts_for_Daryl/21_H_SMA_London_zone_EandW_lines.svg",
         plot = SMA_London_zone_EandW_lines_rep,
         device = "svg",
         dpi=600,
         width = 9,
         height = 5.56,
...
## Scatterplot of TFR in 2021 v. SMA in 2021 for individual London boroughs
...
ggsave (filename = (paste0(chart_dir, "TFR_vSMA_2021_scatterplot.png")),
         plot = TFR_vSMA_2021_scatterplot,
         device = "png",
         dpi=600,
         width = 9,
         height = 5.56,
...
SMA_absChange_map2 <- SMA_absChange_geog %>%
  ...
  theme(legend.key.height= unit(1, 'cm'),
        legend.key.width= unit(0.7, 'cm')) +
  scale_fill_manual("Change in SMA", values = catgreen4) + 
...
```{r fig_London_SMA_10_B, fig.height = 5, fig.width = 9,  echo=FALSE, warning=FALSE, message=FALSE}


SMA_absChange_map2_rep <- SMA_absChange_geog %>%
  ggplot()+
  geom_point(aes(x=BNG_E, y=BNG_N, 
                 text = paste("Borough: ", gss_name,  
                              "<br>SMA in 1993: ", 
...
#       caption = paste0("Source: Home Office, Chart: GLA demography")) 
#  labs(fill = "Change in SMA")
SMA_absChange_map2_rep

ggsave (filename = "C:/Families/Charts_for_Daryl/21_I1_SMA_absChange_map2.svg",
         plot = SMA_absChange_map2_rep,
         device = "svg",
         dpi=600,
         width = 9,
         height = 5.56,
